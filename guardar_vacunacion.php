...
$id_nino = $_POST['id_nino'] ?? null; // Cambiado de id_infante a id_nino para coincidir con el formulario
$tipo_id = $_POST['tipo_id'] ?? null;
$id_personal = $_POST['id_personal'] ?? null;
$fecha_administracion = $_POST['fecha_administracion'] ?? null;

// Validar que todos los datos estén presentes
if (!$id_nino || !$tipo_id || !$id_personal || !$fecha_administracion) {
...
try {
    ...
    $stmt_dosis_actual->bind_param("ii", $id_nino, $tipo_id);
    ...
    $stmt_insert->bind_param("iiisi", $id_nino, $tipo_id, $numero_dosis, $fecha_administracion, $id_personal);

    if ($stmt_insert->execute()) {
        // Redirigir de vuelta a la página de información del infante después de guardar la vacunación
        header("Location: informacion_infante.php?id=$id_nino");
        exit();
    } else {
